...
def main():
    ...
    train_set = PairedImageDataset(cfg['train_input_dir'], cfg['train_target_dir'], cfg['train']['patch_size'], augment=True)
    val_set = PairedImageDataset(cfg['val_input_dir'], cfg['val_target_dir'], patch_size=None, augment=False)
    ...
    while it < cfg['train']['iterations']:
        for batch in train_loader:
            ...
            pbar.update(1)
            pbar.set_description(f"loss={loss.item():.4f} c={logs['charb']:.4f} d={logs['decouple']:.4f}")
            ...
            if it % cfg['train']['save_every'] == 0:
                torch.save({'model': model.state_dict(), 'config': cfg}, os.path.join(save_dir, f'imdnet_iter_{it}.pth'))
